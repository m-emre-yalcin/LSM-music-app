...
export default {
  ...
  async mounted() {
    ...
    this.$store.state.musicDirectories =
      await this.$db.musicDirectories.toArray();
    ...
    setTimeout(() => {
      document.querySelectorAll(".main-glass").forEach((dom) => {
        dom.style =
          this.$store.state.themes[Number(localStorage.getItem("theme"))];
      });
    }, 100);
  },
  methods: {
...
.music-list-container {
  ...
  .music-left-menu {
    ...
    z-index: 2;
    transition: left 0.2s $cubic-bezier-1;
    width: 50px;
    ...
    [class*="group"] {
      ...
      .item {
        ...
        &::after {
          ...
          transition: opacity 0.1s $cubic-bezier-1;
          position: absolute;
          left: 45px;
          min-width: 30px;
          max-width: auto;
